...
switch ($PSCmdlet.ParameterSetName) {
    ...
    'Parallel' {
        ...
        try {
            ...
            Write-Log Error "Failed to import CSV '$CsvPath': $($_.Exception.Message)"
            throw
        }
        foreach ($col in 'Name','ResourceGroup') {
            if (-not ($csv | Get-Member -Name $col -MemberType NoteProperty)) {
                Write-Log Error "CSV missing required column '$col'"
        ...
        while (Get-Job -State Running) {
            ...
            Get-Job | Select-Object Id,Name,State | Format-Table 
        }
